async function handler(m, { command }) {
    ...
    switch (command) {
        ...
        case 'start': {
            ...
            if (room) {
                await this.sendButton(room.a, 'Sukses Menemukan teman untuk kamu, Mulai lah berbicara dengan orang tidak di kenal\n\nKatakan Hallo\n*❏ NOTE:* Jika pesan kamu tidak di balas kemungkinan teman kamu sedang offline/Sibuk', 'Menunggu Balasan...', `${imgr + 'Anonymous Chat'}`, [['CARI TEMAN LAIN', `.next`],['SEND KONTAK', '.sendkontak']], fpay)
                room.b = m.sender
                room.state = 'CHATTING'
                await this.sendButton(room.a, 'Sukses Menemukan teman untuk kamu, Mulai lah berbicara dengan orang tidak di kenal\n\nKatakan Hallo\n*❏ NOTE:* Jika pesan kamu tidak di balas kemungkinan teman kamu sedang offline/Sibuk', 'Menunggu Balasan...', `${imgr + 'Anonymous Chat'}`, [['CARI TEMAN LAIN', `.next`],['SEND KONTAK', '.sendkontak']], fpay)
            } else {
